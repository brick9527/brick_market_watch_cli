...
const NoticeConfig = require('./notice_config');
const ScheduleConfig = require('./schedule_config');
...
class BrickProcess {
  ...
  constructor(process, processName = 'default') {
    ...
    const configEntry = new Config();
    const noticeConfigEntry = new NoticeConfig();
    const scheduleConfigEntry = new ScheduleConfig();
    const proxyConfigEntry = new ProxyConfig();
    ...
    this._brickMarketWatchCli = {
      ...
      class: {
        config: configEntry,
        noticeConfig: noticeConfigEntry,
        scheduleConfig: scheduleConfigEntry,
        proxyConfig: proxyConfigEntry,
      },
      
      ctx: {
        config: configEntry.getJSON(),
        noticeConfig: noticeConfigEntry.getJSON(),
        scheduleConfig: scheduleConfigEntry.getJSON(),
        proxyConfig: proxyConfigEntry.getJSON(),
